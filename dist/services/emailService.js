...
class EmailService {
    constructor() {
        if (!process.env.SMTP_USER || !process.env.SMTP_PASS) {
            logger_1.logger.warn('Email service not configured. SMTP_USER and SMTP_PASS environment variables are required.');
    ...
    async sendEmail(options) {
        try {
            if (!this.transporter) {
                logger_1.logger.warn('Email service not configured. Skipping email send.', { to: options.to, subject: options.subject });
                return false;
            }
            const mailOptions = {
